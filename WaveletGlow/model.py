...
import numpy as np
from Wavelets.Haar.squeezeSplit import SqueezeSplit
from WaveletGlow.flowUnit import FlowUnit
from torch.autograd import Variable
import math


class WaveletGlow(nn.Module):
        ...
        if reverse:
            ...
            for level in range(self.n_levels, self.base_level - 1, -1):
                ...
                if level == partial_level or partial_level == -1:
                    ...
                    haar_ldj = torch.full(ld.shape, np.log(0.5) * (self.n_levels - level), device=details.device, requires_grad=True)
