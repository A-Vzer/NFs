...
class Isic:
    def __init__(self, dataroot, dim=64):
        ...
        self.train_dataset = ISICDataset(path, transform=transform, test=False, malignant=False)
        self.test_dataset = ISICDataset(path, transform=transform, test=True, malignant=False)
        self.out_dataset = ISICDataset(path, transform=transform, malignant=True)
        self.imDim = (3, dim, dim)
        self.num_classes = 2


class ISICDataset(Dataset):
    def __init__(self, img_path, transform, test=False, malignant=False):
        if malignant:
            ...
            if test:
                self.img_path = os.path.join(img_path, "benign_test")
            else:
