...
class FlowUnit(nn.Module):
    ...
    def latent_log_density(self, latent):
        ...
        total_log_density = torch.sum(element_log_density, dim=[1, 2, 3])
    ...
    def forward(self, x, logdet, conditioning=None, reverse=False):
        ...
        if reverse:
            ...
            z, logdet = self.step.forward(x, logdet, conditioning, reverse)
            ld_base = self.latent_log_density(z)
            # ldj = - ldj_base - logdet
            # ld = ld_base + ldj
            return z
        else:
            z, logdet = self.step.forward(z, logdet, conditioning)
            ld = self.latent_log_density(z)
            # z, b_logdet = self.base_tform.forward(z)
            # ld = self.latent_log_density(z)
            # logdet = logdet + b_logdet
            return z, logdet + ld
